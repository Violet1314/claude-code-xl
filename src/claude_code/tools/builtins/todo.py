...
class TodoUpdateTool(Tool):
    ...
    description = (
        ...
        "约束: pending必须先转in_progress才能completed/failed; in_progress可回退pending; completed/failed不可变更。\n"
        "批量示例: TodoUpdate(updates=[{\"id\":\"t1\",\"status\":\"in_progress\"},{\"id\":\"t2\",\"status\":\"in_progress\"}])  单任务: TodoUpdate(id=\"t1\", status=\"completed\")"
    )
