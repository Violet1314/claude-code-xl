这是基于 v2.8.3 格式更新的 **README.md**，已同步至 **v2.8.4** 版本。

主要变更点：
*   **版本号**：更新至 `v2.8.4`。
*   **工具显示格式统一**：所有工具采用统一的标题行 + 分隔线 + 内容格式。
*   **格式规范**：`✎ 工具名: 目标对象 [状态] (元信息)` + 分隔线 + 带行号的内容。
*   **Bash 混合方案**：标题行用统一格式，输出内容用 Panel 包裹。
...
### v2.8.4 (2026-04-07)
**工具显示格式统一重构**
*   ✅ 统一格式规范：`✎ 工具名: 目标对象 [状态] (元信息)` + 分隔线 + 带行号内容
*   ✅ Glob/Grep/Write/Read/Edit：统一 `_build_terminal_display` 格式
*   ✅ Bash 混合方案：统一标题行 + Panel 包裹输出
*   ✅ Edit diff 显示：删除行红色字体，新增行绿色字体
*   ✅ 工具间隔优化：每个工具输出前添加空行分隔
*   ✅ 移除 permission_ui.py 中 `show_tool_start` 的重复显示
*   ✅ 全量测试通过：95 passed
