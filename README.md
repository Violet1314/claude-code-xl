...
### v2.8.18 (2026-04-18)
**终端 UI 优雅化 — 对标官方 Claude Code 设计风格**
*   ✅ 删除 `ℹ 继续处理...`：无信息量的过渡语，工具间由 Spinner + 摘要行自然分隔
*   ✅ Read 输出摘要化：终端只显示一行摘要 `📖 Read filename  N lines · X.X KB`，文件内容仍完整传给模型
*   ✅ 工具结果分组打印：连续同类工具紧凑排列（无空行），不同工具间空行分隔
*   ✅ 跨轮次连续追踪：`_last_displayed_tool` 追踪上一轮工具名，跨轮次连续 Read 也紧凑排列
*   ✅ AI Panel 间距优化：有工具调用时 Panel 不打前后空行，由分组打印统一控制间距
*   ✅ 全量测试通过：178 passed
