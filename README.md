...
### v2.8.14 (2026-04-14)
**Token/费用统计修复**
*   ✅ Token 累计显示：改用 API 真实 usage 累加，稳定增长
*   ✅ 价格解析修复：正则改为 `r'\$(\d+)'`，支持 `$` 在前格式
*   ✅ 删除估算覆盖：不再每轮调用 `update_input()` 覆盖累计值
*   ✅ 状态栏效果：`◆ KIMI K2.6 CODE │ ◆ 3.6K tokens │ $ 0.025`
*   ✅ 全量测试通过：100 passed

### v2.8.13 (2026-04-13)
**权限系统优化 + Read/Edit 数据一致性**
*   ✅ 全局会话放行：选择"允许(会话)"后，所有工具、所有路径自动通过
*   ✅ Read/Edit 一致性：保留原始内容（空格行、行尾空格），换行处理统一
*   ✅ 精确匹配修复：模型从 Read 复制内容可直接用于 Edit 匹配
*   ✅ 全量测试通过：100 passed
